...
class ChatCompletions(classes.Model):
    ...
    def generate_response(self, conversation: classes.Conversation) -> classes.AntonMessage:
        ...
        constants.MAIN_LOG.log(Info("Generating response using Chat Completions interface."))
        message_history = []
        ...
        response = requests.post(self.source_url, headers=headers, json=payload)
        if response.status_code != 200:
            constants.MAIN_LOG.log(Error(f"Error from Chat Completions API: {response.status_code} - {response.text}"))
            raise Exception(f"Chat Completions API error: {response.status_code}")
        response_data = response.json()

        constants.MAIN_LOG.log(Info("Received response from Chat Completions interface."))
        constants.MAIN_LOG.log(Debug(f"Chat Completions response content: {response_data}"))
